...
Params.r  =  4;     % Rank
%Params.m = ceil(4 * Params.n);     % Number of measurements
Params.m = ceil(4 * Params.n);
...
Paramstwf.npower_iter = 300;
Paramstwf.n = Params.n;
Paramstwf.mu = 0.2;
Paramstwf.T = 300;
Paramstwf.m = Params.m;
%err_rwf = zeros(Paramstwf.T, Params.q);
%%Params.seed = rng;
...
%err_X_rwf_iter = zeros(Paramstwf.T, Params.Tmont);
...
time_RWF = zeros(Paramstwf.T, Params.Tmont);
...
% [x_rwf, err_rwf(:, nk), time_temp]  = TWF(Y(:,nk), Paramstwf, A1, At, X(:,nk));
% time_RWF(:, t) = time_RWF(:, t) + time_temp';
        X_rwf(:,nk)  =  x_rwf;
    end
...
semilogy(time_RWF2, final_err_X_rwf2,  'gd--', 'LineWidth', 2)
semilogy(mean(time_RWF1, 2), final_err_X_rwf1(1, :),  'ko--', 'LineWidth', 2)
axis tight
stry = ['mat-dist', '$$(\hat{X}^t, X)$$'];
xlabel('time', 'Fontsize', 15)
ylabel(stry, 'Interpreter', 'latex', 'Fontsize', 15)
l1 = legend('AltminLowRap', 'LRPR2', 'Proj-RWF', 'RWF');
set(l1, 'Fontsize', 15)
t1 = title('m = 150, n = 600, q = 1000');
set(t1, 'Fontsize', 15)
...
% t1 =mean(time_QR, 2);
% t2 = mean(time_OLD, 2);
% loglog(lrpr_suc_time2(1:4), lrpr_suc_err2(1:4), 'rs--', 'LineWidth', 2)
loglog(lrpr_suc_time4(1:end-2), lrpr_suc_err4(1:end-2), 'rs--', 'LineWidth', 2)
hold
loglog(rwf_suc_time1(1:37), rwf_suc_err1(1:37), 'b>--', 'LineWidth', 2)
loglog(rwf_fail_time1, rwf_fail_err1,  'md--', 'LineWidth', 2)
loglog(twf_suc_time1(1:80), twf_suc_err1(1:80),  'ko--', 'LineWidth', 2)
axis tight
stry = ['mat-dist', '$$(\hat{X}^t, X)$$'];
xlabel('time', 'Fontsize', 15)
ylabel(stry, 'Interpreter', 'latex', 'Fontsize', 15)
...
% t2 = mean(time_OLD, 2);
% loglog(lrpr_suc_time2(1:4), lrpr_suc_err2(1:4), 'rs--', 'LineWidth', 2)
semilogy(lrpr_suc_time4(1:end-2), lrpr_suc_err4(1:end-2), 'rs--', 'LineWidth', 2)
hold
semilogy(rwf_suc_time1(1:37), rwf_suc_err1(1:37), 'b>--', 'LineWidth', 2)
semilogy(rwf_fail_time1, rwf_fail_err1,  'gd--', 'LineWidth', 2)
semilogy(twf_suc_time1(1:80), twf_suc_err1(1:80),  'ko--', 'LineWidth', 2)
axis tight
stry = ['mat-dist', '$$(\hat{X}^t, X)$$'];
xlabel('time', 'Fontsize', 15)
ylabel(stry, 'Interpreter', 'latex', 'Fontsize', 15)
